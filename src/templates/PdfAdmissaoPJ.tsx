...
interface IDadosAdmissaoPJ {
  razaoSocial: string; cnpj: string; endereco: string; municipio: string; cep: string; telefone: string; email: string;
...
export const PdfAdmissaoPJ: React.FC<{ data: IDadosAdmissaoPJ }> = ({ data }) => {
  ...
  return (
    <Document>
      <Page size="A4" style={styles.page}>
        <Text style={styles.header}>REQUERIMENTO DE INGRESSO/PERMANÊNCIA NO QUADRO DE ASSOCIADOS(AS) DA ASSOCIAÇÃO DE MORADORES E PROPRIETÁRIOS DO DISTRITO RECANTO MAESTRO – AMPRM</Text>
        <Text style={styles.paragraph}><Text style={styles.bold}>{data.razaoSocial ? data.razaoSocial.toUpperCase() : '___________________________________________'}</Text>, pessoa jurídica de direito privado, inscrita no CNPJ sob o n.° {data.cnpj}, situada na Rua {data.endereco}, em {data.municipio} (Município), CEP {data.cep}, telefone {data.telefone}, e-mail {data.email}, vem através deste <Text style={styles.bold}>REQUERIMENTO</Text> manifestar à Associação de Moradores e Proprietários do Distrito Recanto Maestro, inscrita no CNPJ n.º 15.394.096/0001-97, o interesse de compor o quadro de associados, tendo em vista ser:</Text>
        ...
        <View style={styles.signatureSection}><View style={styles.line} /><Text style={styles.bold}>{data.razaoSocial ? data.razaoSocial.toUpperCase() : ''}</Text><Text style={styles.signatureText}>CNPJ {data.cnpj}</Text></View>
